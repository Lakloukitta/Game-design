...
static collision_info_t compare_collision(list_t *shape1, list_t *shape2,
  ...
  for (size_t i = 0; i < list_size(edges1); i++) {
    vector_t edge_vec = *(vector_t *)list_get(edges1, i);
    vector_t perp_axis = vec_rotate(edge_vec, M_PI / 2);
    vector_t unit_perp = vec_multiply(1 / vec_get_length(perp_axis), perp_axis);

    vector_t projections1 = get_max_min_projections(shape1, unit_perp);
    vector_t projections2 = get_max_min_projections(shape2, unit_perp);
    if (projections1.y <= projections2.x && projections1.y >= projections2.y){
      overlap = projections2.x - projections1.y;
    }
    else {
      overlap = projections1.x - projections2.y;
    }
    ...
    if ((projections2.y > projections1.x) ||
        (projections2.x < projections1.y)) {
      list_free(edges1);
      list_free(edges2);
