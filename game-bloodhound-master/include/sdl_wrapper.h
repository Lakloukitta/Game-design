 ...
 */
typedef enum { KEY_PRESSED = 0, KEY_RELEASED = 1, BUTTON = 2 } key_event_type_t;

typedef enum { NONE = 0, POINTS=1, ROUNDS=2, TIME =3, AVG=4 } value_type_t;
typedef enum { CONTINUE = 0, RESTART=1, RESUME=2, ADVANCED=4, RETURN=5, UNPAUSE=6 } func_type_t;
...
/**
 * Creates an SDL_Surface which can then be passed to
 * sdl_text_texture to create the text texture.
 *
 * @param message the string of text you want to display
 * @param font a TTF_Font pointer to set the font of text
 * @param color an SDL_Color type that determines the color of text
 * @return pointer to an SDL_Surface
 */
SDL_Surface *sdl_make_surface(const char *message, TTF_Font *font,
                              SDL_Color color);

/**
 * Creates an SDL_Texture which can then be passed to
 * sdl_render_image to display the text.
 *
 * @param surface_message an SDL_Surface type of the text
 * @return pointer to an SDL_Texture
 */
SDL_Texture *sdl_text_texture(SDL_Surface *surface_message);

/**
* Converts a color_t to an SDL_Color which is compatible
* with the sdl library for rending functions.
*
* @param color a color_t
* @return an SDL_Color object
*/
SDL_Color sdl_convert_color(color_t color);

/**
 * Creates the SDL_Rect bounding box for the
 * corresponding body that you want to attach the image
 * onto.
 * 
 * @param body a body_t
 * @return an SDL_Rect object
 */
